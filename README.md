...
* 위 식은 현재 속도(`v_cmd`)로 주행할 경우의 예상 도착 시간(`my_eta`)을, 곱한 후, margin_time으로 나누어 정적 속도 할당이 아닌, 상황에 따라 속도가 달라지는 동적할당 방식을 사용하였다.
---
...
* **차별점**: 문제 2에서는 고정된 단일 경로가 아닌, State Machine에 의해 선택된 현재 차선(Active Lane)의 waypoint를 실시간 목표점으로 사용한다.
...
```bash
docker build --no-cache -t smyd:jetson .
```
...
```bash
docker run --rm --net=host --ipc=host -e PROBLEM_ID=1 smyd:jetson
```


### 문제 1-2에 사용하는 알고리즘 (PROBLEM_ID=2)
```bash
docker run --rm --net=host --ipc=host -e PROBLEM_ID=2 smyd:jetson
```

### 문제 2에 사용하는 알고리즘 (PROBLEM_ID=3)
```bash
docker run --rm --net=host --ipc=host -e PROBLEM_ID=3 smyd:jetson
```

### 문제 3에 사용하는 알고리즘 (PROBLEM_ID=4)
```bash
docker run --rm --net=host --ipc=host -e PROBLEM_ID=4 smyd:jetson
```

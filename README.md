...
- **가속 모드**: 충돌 위험이 낮고 안전거리가 충분한 경우, 속도를 최대 **2.0 m/s**로 높여 주행하며 합류한다.
...
문제 1-1의 의존성에 더해 다음과 같은 패키지가 추가되었다.

### ROS 2 Packages

- **domain_bridge**: 시뮬레이터와 제어 노드(CAV) 간의 토픽 통신 중계
- **pkg_p2**: 다중 차선 데이터 파일 및 제어 로직 포함
...
- **Difference**: 문제 2에서는 고정된 경로가 아닌, State Machine에 의해 선택된 현재 차선(Active Lane)의 waypoints를 실시간 목표점으로 설정한다.

### 2. Multi-Lane Data Management

장애물 회피 및 최단 시간 경로 확보를 위해 3개의 차선 데이터를 모두 메모리에 로드하여 관리한다.

- 초기화 단계에서 1, 2, 3차선 경로 데이터를 파싱하여 리스트에 저장한다.
- 사전에 검증된 차선 경로(`json`)를 따름으로써, 주행 불가 영역 침범을 원천적으로 차단한다.

### 3. Frenet Coordinate Estimation

트랙 내에서의 정확한 위치 파악과 장애물 회피 시점 결정을 위해, Global 좌표를 **Frenet 좌표계(s, d)** 로 변환한다.

- 현재 차량 위치를 트랙 진행 거리(s)와 횡방향 오차(d)로 변환한다.
- 계산된 s 값은 장애물 구간을 인식하고 차선을 변경하는 State Machine의 Trigger로 사용된다.

### 4. State Machine (Overtaking & Merging Strategy)

주행 거리(s)에 따라 주행 상태(`state`)를 바꾸며 우선순위(Priority)에 따라 목표 차선을 결정하는 로직을 수행한다.

- **Trigger Condition**: 전방 차량과의 거리가 설정값(`follow_dist`)보다 가까워지면 차선 변경을 시도한다.
- **Lane Priority (Overtaking)**:
    1. **High Priority (Lane 3, 2)**: 주행 가능하다면 Lane 3(Index 2)와 Lane 2(Index 1)를 우선적으로 선택하여 추월 및 고속 주행을 시도한다.
    2. **Low Priority (Lane 1)**: Lane 1(Index 0)은 Lane 2, 3가 모두 불가능한 경우에만 예외적으로 선택하거나, 혹은 현재 차선을 유지하며 감속하는 보수적인 전략을 취한다.
- **Merge Constraint**: 합류 구간에 접근(`merge_approach_lock`)할 때 차량이 Lane 1에 있다면, 강제로 Lane 2로 변경하여 합류 사고를 예방한다.

### 5. Adaptive Speed Control

급격한 차선 변경이나 회피 기동 시 차량의 안정성을 확보하기 위해 조향각에 따라 속도를 능동적으로 조절한다.

- $v_{target} = \frac{v_{ref}}{1.0 + k \times |\delta|}$
    - 조향각(delta)이 클수록 목표 속도를 낮춰 전복이나 미끄러짐을 방지한다.
    - 직선 구간에서는 최고 속도를 유지하여 '최단 시간 주행' 목표를 달성한다.
## 환경구성
